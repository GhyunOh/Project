...
def sliding_window(data_dir, datatype, window_size, sample_ratio=1):
    ...
    dataset structure
        ...
        labels(list)
    '''
    event2semantic_vec = read_json(data_dir + 'hdfs/AWS_vec.json')
    num_sessions = 0
    result_logs = {}
    ...
    with open(data_dir, 'r') as f:
        for line in f.readlines():
            ...
            print(window_size,len(line), num_sessions, data_dir)
            for i in range(len(line) - window_size):
                Sequential_pattern = list(line[i:i + window_size])
                Quantitative_pattern = [0] * 122
...
def session_window(data_dir, datatype, sample_ratio=1):
    ...
    if datatype == 'train':
        data_dir += 'hdfs/train.csv'
    elif datatype == 'val':
        data_dir += 'hdfs/valid.csv'
    elif datatype == 'test':
        data_dir += 'hdfs/test.csv'
